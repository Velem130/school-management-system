...
function MenDistribution() {
  const [teachers, setTeachers] = useState([]);
  const [students, setStudents] = useState([]);
  const [selectedTeacher, setSelectedTeacher] = useState(null);
  const [distributionType, setDistributionType] = useState("food_hamper");
  ...
  const loadDistributions = () => {
    const saved = localStorage.getItem('menDistributions');
    if (saved) {
  ...
  const saveDistributions = (newDistributions) => {
    localStorage.setItem('menDistributions', JSON.stringify(newDistributions));
  ...
  const handleClearAll = () => {
    if (!window.confirm(
      `Clear all ${distributionType === 'food_hamper' ? 'Food Hamper' : 'Shoes'} distributions for ${selectedTeacher?.name || 'all members'}?\n\n` +
  ...
  const generatePDF = async () => {
    ...
    try {
          ...
          <div style="width: 100%; max-width: 190mm; margin: 0 auto;">
            ${pageNum === 0 ? `
              <div style="border-bottom: 2px solid #000; padding-bottom: 3px; margin-bottom: 4px;">
                <div style="display: flex; justify-content: space-between; align-items: flex-end;">
                  <div>
                    <h1 style="margin: 0; font-size: 16px; font-weight: bold; color: #000;">BBIC Management — Men's ${distributionLabel} Distribution</h1>
            ...
            ` : `
              <div style="border-bottom: 1px solid #000; padding-bottom: 2px; margin-bottom: 4px; display: flex; justify-content: space-between;">
                <p style="margin: 0; font-size: 9px; color: #000;"><strong>${selectedTeacher.name}</strong> — Men's ${distributionLabel} (continued)</p>
            ...
            ${pageNum === totalPages - 1 ? `
              <p style="margin-top: 6px; font-size: 8px; color: #555; text-align: right;">
                Distribution Summary: ${receivedStudents.length} received, ${pendingStudents.length} pending — ${sortedStudents.length} member(s) total
              </p>
            ` : ''}
          </div>
        `;
      ...
      const fileName = `${selectedTeacher.name.replace(/\s+/g, '_')}_Mens_${distributionLabel.replace(/\s+/g, '_')}_Distribution_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);
  ...
  return (
    <div className="pt-10 pb-10 px-4 md:px-8">
      <h1 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">Men's Distribution Tracking</h1>

      {/* Global Statistics Banner */}
      <div className="bg-gradient-to-r from-indigo-600 to-blue-700 text-white rounded-xl shadow-lg p-4 md:p-6 mb-6">
        <h2 className="text-lg md:text-xl font-bold mb-3">
          📊 Global Men's Distribution Stats - {distributionType === 'food_hamper' ? '🛒 Food Hampers' : '👟 Shoes'}
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
            <div className="text-2xl md:text-3xl font-bold">{students.length}</div>
            <div className="text-xs md:text-sm opacity-90">Total Members</div>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {loading ? (
              ...
              <>
                {teachers.map((t) => {
                  const memberCount = students.filter(s => s.ustadh === t.name).length;
                ...
                {teachers.length === 0 && (
                  <div className="text-center text-gray-500 py-4 text-sm">
                    No men's teachers registered yet
                  </div>
                )}
              </>
            )}
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-3">
            <div>
              <h2 className="font-semibold text-base md:text-lg">
                {selectedTeacher.name}'s Members - {distributionType === 'food_hamper' ? 'Food Hampers' : 'Shoes'}
              </h2>
              <p className="text-sm text-gray-600">
                {receivedCount} of {teacherStudents.length} members received
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-indigo-600 text-white px-3 md:px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium"
              >
                {isGeneratingPDF ? (
                  <>
              ...
              <thead className="bg-gray-100 text-gray-600">
                <tr>
                  <th className="p-3 text-left">
                    <input type="checkbox" className="h-4 w-4 opacity-0 cursor-not-allowed" disabled />
                  </th>
                ...
                {sortedStudents.map((student) => {
                  ...
                  return (
                    <tr key={student.id} className={`border-b ${received ? 'bg-green-50' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className="p-3">{student.cell || '-'}</td>
                      <td className="p-3">
                        {received ? (
                ...
                {sortedStudents.length === 0 && (
                  <tr>
                    <td colSpan="8" className="p-4 text-center text-gray-500">
                      {studentSearch ? "No members found matching your search" : "No members in this class"}
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    <input
                      ...
                      checked={received}
                      onChange={() => handleToggleDistribution(student.id)}
                      className="h-5 w-5 cursor-pointer text-indigo-600 mt-1"
                    />
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 space-y-1 mt-2">
                        ...
                        <p><span className="font-medium">Cell:</span> {student.cell || '-'}</p>
                        {details && (
          ...
          {sortedStudents.length > 0 && (
            <div className="mt-6 p-4 bg-gradient-to-r from-indigo-50 to-blue-50 rounded-lg border border-indigo-200">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-indigo-700">{teacherStudents.length}</div>
      ...
      {!selectedTeacher && (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <p className="text-gray-600">
            Select a teacher/class and distribution type above to start tracking men's distributions
          </p>
        </div>
      )}
    </div>
...
export default MenDistribution;