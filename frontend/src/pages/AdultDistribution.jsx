...
function AdultDistribution() {
  ...
  const loadDistributions = () => {
    const saved = localStorage.getItem('adultDistributions');
    if (saved) {
  ...
  const saveDistributions = (newDistributions) => {
    localStorage.setItem('adultDistributions', JSON.stringify(newDistributions));
  ...
  const generatePDF = async () => {
    ...
    try {
          ...
          <div style="width: 100%; max-width: 190mm; margin: 0 auto;">
            ${pageNum === 0 ? `
              <div style="border-bottom: 2px solid #000; padding-bottom: 3px; margin-bottom: 4px;">
                <div style="display: flex; justify-content: space-between; align-items: flex-end;">
                  <div>
                    <h1 style="margin: 0; font-size: 16px; font-weight: bold; color: #000;">BBIC Management — Adult ${distributionLabel} Distribution</h1>
            ...
            ` : `
              <div style="border-bottom: 1px solid #000; padding-bottom: 2px; margin-bottom: 4px; display: flex; justify-content: space-between;">
                <p style="margin: 0; font-size: 9px; color: #000;"><strong>${selectedTeacher.name}</strong> — Adult ${distributionLabel} (continued)</p>
            ...
            ${pageNum === totalPages - 1 ? `
              <p style="margin-top: 6px; font-size: 8px; color: #555; text-align: right;">
                Distribution Summary: ${receivedStudents.length} received, ${pendingStudents.length} pending — ${sortedStudents.length} learner(s) total
              </p>
            ` : ''}
          </div>
        `;
      ...
      const fileName = `${selectedTeacher.name.replace(/\s+/g, '_')}_Adult_${distributionLabel.replace(/\s+/g, '_')}_Distribution_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);
  ...
  return (
    <div className="pt-10 pb-10 px-4 md:px-8">
      <h1 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">Adult Distribution Tracking</h1>

      {/* Global Statistics Banner */}
      <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl shadow-lg p-4 md:p-6 mb-6">
        <h2 className="text-lg md:text-xl font-bold mb-3">
          📊 Global Adult Distribution Stats - {distributionType === 'food_hamper' ? '🛒 Food Hampers' : '👟 Shoes'}
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
            <div className="text-2xl md:text-3xl font-bold">{students.length}</div>
            <div className="text-xs md:text-sm opacity-90">Total Learners</div>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
              ...
              <>
                ...
                {teachers.length === 0 && (
                  <div className="text-center text-gray-500 py-4 text-sm">
                    No adult teachers registered yet
                  </div>
                )}
              </>
            )}
      ...
      {/* Learner Distribution List */}
      {selectedTeacher && (
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-3">
            <div>
              <h2 className="font-semibold text-base md:text-lg">
                {selectedTeacher.name}'s Learners - {distributionType === 'food_hamper' ? 'Food Hampers' : 'Shoes'}
              </h2>
              <p className="text-sm text-gray-600">
                {receivedCount} of {teacherStudents.length} learners received
              </p>
            </div>
                ...
                {sortedStudents.length === 0 && (
                  <tr>
                    <td colSpan="8" className="p-4 text-center text-gray-500">
                      {studentSearch ? "No learners found matching your search" : "No learners in this class"}
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    <input
                      ...
                      checked={received}
                      onChange={() => handleToggleDistribution(student.id)}
                      className="h-5 w-5 cursor-pointer text-purple-600 mt-1"
                    />
          ...
          {sortedStudents.length > 0 && (
            <div className="mt-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-purple-700">{teacherStudents.length}</div>
      ...
      {!selectedTeacher && (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <p className="text-gray-600">
            Select a teacher/class and distribution type above to start tracking adult distributions
          </p>
        </div>
      )}
    </div>
...
export default AdultDistribution;